...
namespace DaydreamElements.ObjectManipulation {
  ...
  public class PhysicsPickup : BaseInteractiveObject, IPointerDownHandler {
    ...
    protected override void OnSelect(){
      ...
      if(d >= minDistance && d <= maxDistance){
		if (!holdObject && keepObject){
			KeepObject();
		}
		player.SendMessage("HoldingObject");
		if (rigidbodyCmp.IsSleeping()){
			rigidbodyCmp.WakeUp();
		}
    ...
    protected override void OnDeselect(){
	  ...
	  player.SendMessage("NotHoldingObject");
    }
    ...
    private void UpdateControlTransform() {
	  ...
	  // If we have pulled the object close enough to keep it...
		if (targetToControl.magnitude < keepDistance && keepObject){
			KeepObject();
		}
    }
    ...
    private void ResetRigidbody() {
	  ...
	  if (throwMultiplier > 0.0f){
		rigidbodyCmp.AddForce((_throwVelocity * throwMultiplier), ForceMode.VelocityChange);  
	  }
